function write2ismrmrd(gas_fid,dis_fid,gas_traj,dis_traj,meta);
...
acqblock = ismrmrd.Acquisition(size(gas_fid,2));

% Set the header elements that don't change
acqblock.head.version(:) = 1;
acqblock.head.number_of_samples(:) = size(gas_fid,1);
acqblock.head.center_sample(:) = floor(nX/2);
acqblock.head.active_channels(:) = 1;
acqblock.head.read_dir  = repmat([1 0 0]',[1 size(gas_fid,2)]);
acqblock.head.phase_dir = repmat([0 1 0]',[1 size(gas_fid,2)]);
acqblock.head.slice_dir = repmat([0 0 1]',[1 size(gas_fid,2)]);
